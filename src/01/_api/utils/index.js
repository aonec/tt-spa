...
export function createTimer({
  ...
  const final = formatTime(new Date(start) - new Date(finish));
...
export function changeItemStage(item, i, arr, uos) {
  const {
    status, type, closingTime, expectedCompletionTime, perpetrator,
  } = item;

  const diff = closingTime !== null ? moment(closingTime).format('YYYYMMDD') - moment(expectedCompletionTime).format('YYYYMMDD') : null;

  console.log('diff', diff);

  const icon = status === 'Done' && diff > 0 ? 'timer'
    : status === 'Done' ? 'ok'
      : type === 'Switch'
        ? 'choice'
        : type === 'Final'
