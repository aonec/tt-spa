...
import IconTT from "../../../tt-components/IconTT/index";
...
interface GraphViewProps {
    ...
    searchQuery: RequestNodeReadingsFunctionInterface
    // graphData: ReadingsInterface
}

const GraphView: React.FC<GraphViewProps> = ({graphParam, searchQuery}) => {
    const isFetching = useRef(true);
    const [isPreparing, setIsPreparing] = useState(false);
    // const [newTickValues, setNewTickValues] = useState(initState);
    ...
    const tickValues = useMemo(() => formTicks(archiveEntries, searchQuery.reportType), [archiveEntries]);

    // const tickValues = formTicks(archiveEntries, searchQuery.reportType;

    if (status === 'pending' || status === 'idle') return  <div>ЗАГРУЗКА...</div>
    ...
    const paramKeys = Object.keys(archiveEntries[0]?.values);


    const params = paramKeys.map((key) => ({value: key}))


    const graphData = formGraphData(tickValues, graphParam);
    ...
    return (
        <>
            ...
            {status === 'idle' || status === 'pending' && <div>ЗАГРУЗКА...</div>}

            {/*{status === 'idle' && <div>Start your journey by clicking a button</div>}*/}
            {status === 'resolved' && <GraphWrapper>
