...
class ComponentToPrint extends React.Component {
  render() {
    ...
    const strDate = nowDate.getDate() + months[nowDate.getMonth() - 1] + nowDate.getFullYear(); 
    return (
       <div className="messageBox" style={{left: '454px', top: '66px', padding: '12px 24px', }}>
          ...
          <div style={{textAlign: 'left'}}>
            ...
            <div>от <span style={{textDecoration: 'underline'}}>{strDate}</span></div>
            ...
            <div style={{paddingTop: '24px', borderBottom: '1px solid black', marginLeft: '80px'}}><input type={'text'} defaultValue={fullName}/></div>
            <div style={{clear: 'both', float: 'none', width: '0px', padding: '0px', margin: '0px'}} />
            <div style={{paddingTop: '12px'}}>проживающей(ему) по адресу: <span style={{textDecoration: 'underline'}}> {city} </span>, ул.<span style={{textDecoration: 'underline'}}> {street} </span>, д.<span style={{textDecoration: 'underline'}}> {housingStockNumber} </span>, кв.<span style={{textDecoration: 'underline'}}> {apartmentNumber} </span></div>
            <div style={{paddingTop: '12px'}}>о том, что показания квартирных приборов учета на <span style={{textDecoration: 'underline'}}>{strDate}</span> составляют:</div>
              ...
              <tbody>
                ...
                {device.map(x => (
                  <tr>
                    ...
                    <td style={{padding: '6px', borderRight: '1px solid black', borderBottom: '1px solid black'}}>{x.model}</td>
                    <td style={{padding: '6px', borderRight: '1px solid black', borderBottom: '1px solid black'}}>{x.serialNumber}</td>
                    <td style={{padding: '6px', borderRight: '1px solid black', borderBottom: '1px solid black'}}>{x.reading}</td>
                    <td style={{padding: '6px', borderBottom: '1px solid black'}}>{x.readingDate}</td>
                  </tr>
                ))}
              </tbody>
            </table>
    ...
    <>
      <TitleWrap style={{ paddingTop: '32px' }}>
        ...
        <Certificate certificate={{ 
          fullName: firstName, 
          address:{city: 'Нижнекамск', street: 'Тихая Аллея', housingStockNumber: '4А', apartmentNumber: '143'},
          device:[{resource: 'ХВС', mountPlace: 'ТУ', model: 'СГВ', serialNumber: '123456', reading: '12.34', readingDate: '01.08.2021'},
                  {resource: 'ГВС', mountPlace: 'ТУ', model: 'СГВ', serialNumber: '456789', reading: '87.69', readingDate: '01.08.2021'},
                  {resource: 'Электроэнергия', mountPlace: '--', model: 'Меркурий', serialNumber: '987321', reading: 'День: 15444 Ночь: 5369', readingDate: '01.08.2021'}]}}/>
        </div>
      </TitleWrap>
      <ListItem>
